...
S_AUDIO_FUNC_IDX s_PCM_CH2_FUNC = {
    .Play           =   PCM_CH2_Play,
    .Pause          =   PCM_CH2_Pause,
    .Resume         =   PCM_CH2_Resume,
    .Stop           =   PCM_CH2_Stop,
    .SetVolume      =   PCM_CH2_SetVolume,
    .GetStatus      =   PCM_CH2_GetStatus,
#if _PCM_CH2_SPEED_CHANGE
    .SetSpeed       =   PCM_CH2_SetSpeed,
#endif
#if _PCM_CH2_PITCH_CHANGE
    .SetPitch       =   PCM_CH2_SetPitch,
#endif
#if _PCM_CH2_ROBOT1_SOUND
    .SetRobot1Sound =   PCM_CH2_SetRobot1Sound,
#endif
#if _PCM_CH2_ROBOT2_SOUND
    .SetRobot2Sound =   PCM_CH2_SetRobot2Sound,
#endif
#if _PCM_CH2_ROBOT3_SOUND
    .SetRobot3Sound =   PCM_CH2_SetRobot3Sound,
#endif
#if _PCM_CH2_REVERB_SOUND
    .SetReverbSound =   PCM_CH2_SetReverbSound,
#endif
#if _PCM_CH2_ECHO_SOUND
    .SetEchoSound   =   PCM_CH2_SetEchoSound,
#endif
#if _AUDIO_MIX_CONTROL
    .SetMixCtrl     =   PCM_CH2_SetMixCtrl,
#endif
};
/* Static Functions ----------------------------------------------------------*/
#if _PCM_CH2_RECORD
...
/* Global Variables ----------------------------------------------------------*/
#if _AUDIO_MIX_CONTROL
U8 PcmMixCtrl_CH2=MIX_CTRL_100;
#endif
...
/* Fix RAM--------------------------------------------------------------------*/
#else
static U16 AudioBuf[PCM_CH2_AUDIO_BUF_SIZE];
#if _PCM_CH2_SPEED_CHANGE||_PCM_CH2_PITCH_CHANGE
#if _PCM_CH2_ROBOT3_SOUND
...
extern void CB_PCM_CH2_InitDac(CH_TypeDef *chx,U16 sample_rate,U8 upsample);
extern void CB_PCM_CH2_DacCmd(CH_TypeDef *chx,U8 cmd);
extern void CB_PCM_CH2_ChangeSR(CH_TypeDef *chx,U16 sample_rate);
extern void CB_PCM_CH2_InitAdc(U16 sample_rate);
extern void CB_PCM_CH2_AdcCmd(U8 cmd);
extern void CB_PCM_CH2_WriteDac(U8 size,U16* psrc16);
//------------------------------------------------------------------//
// Allocate PCM memory
// Parameter: 
...
static void PCM_AllocateMemory(U16 func)
{
    ...
    {
        AudioBuf = (U16 *)MemAlloc(PCM_CH2_AUDIO_BUF_SIZE<<1);
#if _PCM_CH2_SPEED_CHANGE||_PCM_CH2_PITCH_CHANGE
#if _PCM_CH2_ROBOT3_SOUND
...
//            *psrc16++=0;            
//        }   
        CB_PCM_CH2_WriteDac(size,(U16 *)psrc16);
...
//            *pdest32=0;           
//        }   
        CB_PCM_CH2_WriteDac(size,(U16 *)0);          
    }                 
}    
#if SMOOTH_ENABLE
//------------------------------------------------------------------//
...
static void DecodeFrame(void)
{
    ...
    while(SpeedCtl.index<SPEED_CTL_FRAME_SIZE){
        ...
        if(AudioBufCtl.sample_count<AudioBufCtl.samples){
            ...
            AudioBufCtl.fifo_count2=AudioBufCtl.fifo_count+(PCM_CH2_AUDIO_BUF_SIZE*2);
        }else{
    ...
    audio_buf=AudioBuf+(PCM_CH2_AUDIO_OUT_SIZE*AudioBufCtl.index);
    
    AudioBufCtl.index++;
    if(AudioBufCtl.index==PCM_CH2_AUDIO_OUT_COUNT){
        AudioBufCtl.index=0;    
    }    
    
    if(SpeedCtl.speed_ratio_index==RATIO_INIT_INDEX && SpeedCtl.pitch_ratio_index==RATIO_INIT_INDEX){
        speed_ctl_size=PCM_CH2_AUDIO_OUT_SIZE;
        for(i=0;i<PCM_CH2_AUDIO_OUT_SIZE;i++){
    ...
    if(RobotCtl.enable){
        if(RobotCtl.change){
            RoboticVoiceInit(AudioBufCtl.sample_rate, CarrierTbl[RobotCtl.index], PCM_CH2_AUDIO_OUT_SIZE);
            //dprintf("RobotCtl:%d\r\n",RobotCtl.carrier);
            RobotCtl.change=0;
        }    
...
#endif
    AudioBufCtl.process_count+=PCM_CH2_AUDIO_OUT_SIZE;
...
//    dprintf("Play:%x,%x,%x\r\n",pheader->signature[0],pheader->signature[1],pheader->signature[2]);
	...
	GIE_DISABLE();
#if	_PCM_CH2_PITCH_CHANGE
    S8 hw_ch = DacAssigner_Regist(kPCM_CH2,PCM_CH2_EXCLUSIVE);
#else
	S8 hw_ch = DacAssigner_Regist(kPCM_CH2,pheader->sample_rate);
#endif
	GIE_ENABLE();
    if(hw_ch<0){
    ...
    AudioBufCtl.out_count=PCM_CH2_AUDIO_BUF_SIZE/AudioBufCtl.out_size;
    AudioBufCtl.upsample=pheader->flag&0x1;
    
    BufCtl.offset=0;
#if _PCM_CH2_SPEED_CHANGE||_PCM_CH2_PITCH_CHANGE
    BufCtl.size=PCM_CH2_AUDIO_BUF_SIZE;
#else    
...
//    }  
	...
	CB_PCM_CH2_InitDac(pHwCtl[hw_ch].p_dac,AudioBufCtl.sample_rate,AudioBufCtl.upsample);
...
#endif 
    CB_PCM_CH2_ChangeSR(pHwCtl[hw_ch].p_dac,AudioBufCtl.sample_rate*PitchRatioTbl[SpeedCtl.pitch_ratio_index]);
#endif         
    ...
    memset((void *)AudioBuf, 0, PCM_CH2_AUDIO_BUF_SIZE*2);
...
#if	!(_PCM_CH2_SPEED_CHANGE||_PCM_CH2_PITCH_CHANGE||_PCM_CH2_ROBOT3_SOUND)
    while((AudioBufCtl.fifo_count+(PCM_CH2_AUDIO_BUF_SIZE-PCM_CH2_AUDIO_OUT_SIZE)-AudioBufCtl.process_count)>=0){
            DecodeFrame();
    }
#else    
    DecodeFrame();
    ...
    CB_PCM_CH2_DacCmd(pHwCtl[hw_ch].p_dac,ENABLE);
...
void PCM_CH2_Resume(void){
	if(AudioBufCtl.state==STATUS_PAUSE&&(!OpMode)){
        if(DacAssigner_codecUseDacCh(kPCM_CH2)<0) {
            GIE_DISABLE();
#if	_PCM_CH2_PITCH_CHANGE
            S8 hw_ch = DacAssigner_Regist(kPCM_CH2,PCM_CH2_EXCLUSIVE);
#else
            S8 hw_ch = DacAssigner_Regist(kPCM_CH2,AudioBufCtl.sample_rate);
#endif
            GIE_ENABLE();
            ...
            CB_PCM_CH2_InitDac(pHwCtl[hw_ch].p_dac,AudioBufCtl.sample_rate,AudioBufCtl.upsample);
#if	_PCM_CH2_PITCH_CHANGE
            PCM_CH2_SetPitch(SpeedCtl.pitch_ratio_index-RATIO_INIT_INDEX);
#endif
            CB_PCM_CH2_DacCmd(pHwCtl[hw_ch].p_dac,ENABLE);
        }
...
#endif
        S8 hw_ch = DacAssigner_codecUseDacCh(kPCM_CH2);
		if(hw_ch<0)return;//No register hw_ch
		CB_PCM_CH2_ChangeSR(pHwCtl[hw_ch].p_dac,AudioBufCtl.sample_rate*PitchRatioTbl[index]);
    }
#endif     
}
//------------------------------------------------------------------//
...
//          NONE
//------------------------------------------------------------------// 
void PCM_CH2_SetRobot1Sound(U8 cmd,U8 index){
#if _PCM_CH2_ROBOT1_SOUND   
...
//          NONE
//------------------------------------------------------------------// 
void PCM_CH2_SetRobot2Sound(U8 cmd,U8 type, U8 thres){
...
//          NONE
//------------------------------------------------------------------// 
void PCM_CH2_SetRobot3Sound(U8 cmd,U8 type, U8 thres){
...
//------------------------------------------------------------------// 
#if _AUDIO_MIX_CONTROL
void PCM_CH2_SetMixCtrl(U8 mixCtrl){
    if(mixCtrl>MIX_CTRL_100) mixCtrl=MIX_CTRL_100;
    PcmMixCtrl_CH2=mixCtrl;
}
#endif
//------------------------------------------------------------------//
// Codec init
...
void PCM_CH2_Init(void){
    ChVol=CH_VOL_15;    
#if _AUDIO_MIX_CONTROL
    PCM_CH2_SetMixCtrl(MIX_CTRL_100);
#endif
#if _PCM_CH2_SPEED_CHANGE||_PCM_CH2_PITCH_CHANGE
...
//    StorageIndex=index;
    ...
    CB_PCM_CH2_InitAdc(sample_rate);
...
#if _PCM_CH2_SPEED_CHANGE||_PCM_CH2_PITCH_CHANGE
        if((AudioBufCtl.fifo_count+(PCM_CH2_AUDIO_BUF_SIZE-PCM_CH2_AUDIO_OUT_SIZE)-AudioBufCtl.process_count)>=0){
...
#endif    
    if(     AudioBufCtl.shadow_state!=STATUS_PLAYING
        &&  DacAssigner_codecUseDacCh(kPCM_CH2)>=0
    ){
		GIE_DISABLE();
		DacAssigner_unRegist(kPCM_CH2);
		GIE_ENABLE();
