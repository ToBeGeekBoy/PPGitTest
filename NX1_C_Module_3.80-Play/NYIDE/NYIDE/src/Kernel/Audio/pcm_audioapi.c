...
S_AUDIO_FUNC_IDX s_PCM_FUNC = {
    .Play           =   PCM_Play,
    .Pause          =   PCM_Pause,
    .Resume         =   PCM_Resume,
    .Stop           =   PCM_Stop,
    .SetVolume      =   PCM_SetVolume,
    .GetStatus      =   PCM_GetStatus,
#if _PCM_SPEED_CHANGE
    .SetSpeed       =   PCM_SetSpeed,
#endif
#if _PCM_PITCH_CHANGE
    .SetPitch       =   PCM_SetPitch,
#endif
#if _PCM_ROBOT1_SOUND
    .SetRobot1Sound =   PCM_SetRobot1Sound,
#endif
#if _PCM_ROBOT2_SOUND
    .SetRobot2Sound =   PCM_SetRobot2Sound,
#endif
#if _PCM_ROBOT3_SOUND
    .SetRobot3Sound =   PCM_SetRobot3Sound,
#endif
#if _PCM_REVERB_SOUND
    .SetReverbSound =   PCM_SetReverbSound,
#endif
#if _PCM_ECHO_SOUND
    .SetEchoSound   =   PCM_SetEchoSound,
#endif
#if _AUDIO_MIX_CONTROL
    .SetMixCtrl     =   PCM_SetMixCtrl,
#endif
};
/* Static Functions ----------------------------------------------------------*/
#if _PCM_RECORD
...
/* Fix RAM--------------------------------------------------------------------*/
#else
static U16 AudioBuf[PCM_AUDIO_BUF_SIZE];
#if _PCM_SPEED_CHANGE||_PCM_PITCH_CHANGE
#if _PCM_ROBOT3_SOUND
...
extern void CB_PCM_InitDac(CH_TypeDef *chx,U16 sample_rate,U8 upsample);
extern void CB_PCM_DacCmd(CH_TypeDef *chx,U8 cmd);
extern void CB_PCM_ChangeSR(CH_TypeDef *chx,U16 sample_rate);
extern void CB_PCM_InitAdc(U16 sample_rate);
extern void CB_PCM_AdcCmd(U8 cmd);
extern void CB_PCM_WriteDac(U8 size,U16* psrc16);
//------------------------------------------------------------------//
// Allocate PCM memory
// Parameter: 
...
static void PCM_AllocateMemory(U16 func)
{
    ...
    {
        AudioBuf = (U16 *)MemAlloc(PCM_AUDIO_BUF_SIZE<<1);
#if _PCM_SPEED_CHANGE||_PCM_PITCH_CHANGE
#if _PCM_ROBOT3_SOUND
...
//            *psrc16++=0;            
//        }   
        CB_PCM_WriteDac(size,(U16 *)psrc16);
...
static void DecodeFrame(void)
{
    ...
    while(SpeedCtl.index<SPEED_CTL_FRAME_SIZE){
        ...
        if(AudioBufCtl.sample_count<AudioBufCtl.samples){
            ...
            AudioBufCtl.fifo_count2=AudioBufCtl.fifo_count+(PCM_AUDIO_BUF_SIZE*2);
        }else{
    ...
    audio_buf=AudioBuf+(PCM_AUDIO_OUT_SIZE*AudioBufCtl.index);
    
    AudioBufCtl.index++;
    if(AudioBufCtl.index==PCM_AUDIO_OUT_COUNT){
        AudioBufCtl.index=0;    
    }    
    
    if(SpeedCtl.speed_ratio_index==RATIO_INIT_INDEX && SpeedCtl.pitch_ratio_index==RATIO_INIT_INDEX){
        speed_ctl_size=PCM_AUDIO_OUT_SIZE;
        for(i=0;i<PCM_AUDIO_OUT_SIZE;i++){
    ...
    if(RobotCtl.enable){
        if(RobotCtl.change){
            RoboticVoiceInit(AudioBufCtl.sample_rate, CarrierTbl[RobotCtl.index], PCM_AUDIO_OUT_SIZE);
            //dprintf("RobotCtl:%d\r\n",RobotCtl.carrier);
            RobotCtl.change=0;
        }    
...
#endif
    AudioBufCtl.process_count+=PCM_AUDIO_OUT_SIZE;
...
//    dprintf("Play:%x,%x,%x\r\n",pheader->signature[0],pheader->signature[1],pheader->signature[2]);
	...
	GIE_DISABLE();
#if	_PCM_PITCH_CHANGE
    S8 hw_ch = DacAssigner_Regist(kPCM_CH1,PCM_CH1_EXCLUSIVE);
#else
	S8 hw_ch = DacAssigner_Regist(kPCM_CH1,pheader->sample_rate);
#endif
	GIE_ENABLE();
    if(hw_ch<0){
    ...
    AudioBufCtl.out_count=PCM_AUDIO_BUF_SIZE/AudioBufCtl.out_size;
    AudioBufCtl.upsample=pheader->flag&0x1;
    
    BufCtl.offset=0;
#if _PCM_SPEED_CHANGE||_PCM_PITCH_CHANGE
    BufCtl.size=PCM_AUDIO_BUF_SIZE;
#else    
...
//    }  
	...
	CB_PCM_InitDac(pHwCtl[hw_ch].p_dac,AudioBufCtl.sample_rate,AudioBufCtl.upsample);
...
#endif
    CB_PCM_ChangeSR(pHwCtl[hw_ch].p_dac,AudioBufCtl.sample_rate*PitchRatioTbl[SpeedCtl.pitch_ratio_index]);
#endif         
    ...
    memset((void *)AudioBuf, 0, PCM_AUDIO_BUF_SIZE*2);
...
#if	!(_PCM_SPEED_CHANGE||_PCM_PITCH_CHANGE||_PCM_ROBOT3_SOUND)
    while((AudioBufCtl.fifo_count+(PCM_AUDIO_BUF_SIZE-PCM_AUDIO_OUT_SIZE)-AudioBufCtl.process_count)>=0){
            DecodeFrame();
    }
#else    
    DecodeFrame();
	...
	CB_PCM_DacCmd(pHwCtl[hw_ch].p_dac,ENABLE);
...
void PCM_Resume(void){
	if(AudioBufCtl.state==STATUS_PAUSE&&(!OpMode)){
        if(DacAssigner_codecUseDacCh(kPCM_CH1)<0) {
            GIE_DISABLE();
#if	_PCM_PITCH_CHANGE
            S8 hw_ch = DacAssigner_Regist(kPCM_CH1,PCM_CH1_EXCLUSIVE);
#else
            S8 hw_ch = DacAssigner_Regist(kPCM_CH1,AudioBufCtl.sample_rate);
#endif
            GIE_ENABLE();
            ...
            CB_PCM_InitDac(pHwCtl[hw_ch].p_dac,AudioBufCtl.sample_rate,AudioBufCtl.upsample);
#if	_PCM_PITCH_CHANGE
            PCM_SetPitch(SpeedCtl.pitch_ratio_index-RATIO_INIT_INDEX);
#endif
            CB_PCM_DacCmd(pHwCtl[hw_ch].p_dac,ENABLE);
        }
...
#endif
        S8 hw_ch = DacAssigner_codecUseDacCh(kPCM_CH1);
		if(hw_ch<0)return;//No register hw_ch
		CB_PCM_ChangeSR(pHwCtl[hw_ch].p_dac,AudioBufCtl.sample_rate*PitchRatioTbl[index]);
    }
#endif     
}
//------------------------------------------------------------------//
...
//          NONE
//------------------------------------------------------------------// 
void PCM_SetRobot1Sound(U8 cmd,U8 index){
#if _PCM_ROBOT1_SOUND   
...
//          NONE
//------------------------------------------------------------------// 
void PCM_SetRobot2Sound(U8 cmd,U8 type, U8 thres){
...
//          NONE
//------------------------------------------------------------------// 
void PCM_SetRobot3Sound(U8 cmd,U8 type, U8 thres){
...
//------------------------------------------------------------------// 
#if _AUDIO_MIX_CONTROL
void PCM_SetMixCtrl(U8 mixCtrl){
...
//          NONE
//------------------------------------------------------------------// 
void PCM_AdcIsr(U8 size, U32 *pdest32){
    S16 *pdst16;
    U8 i;
    U16 temp;
...
//          NONE
//------------------------------------------------------------------// 
void PCM_SkipTailSetting(U16 ms)
{
	SkipTailTime = ms;
}
//------------------------------------------------------------------//
...
//          NONE
//------------------------------------------------------------------// 
static U16 PCM_SkipTail_Samples(U16 now_sample_rate)
{
	U16 skip_samples = (now_sample_rate/1000)*SkipTailTime;//calculate tail samples
	return (skip_samples+39-((skip_samples+39)%40));
}
//------------------------------------------------------------------//
// Write audio header
// Parameter: 
...
static void WriteHeader(void){
    ...
    U16 SkipTailSamples = PCM_SkipTail_Samples(pheader->sample_rate);
    ...
    {
    	pheader->samples=40;//min frame samples
    }
	pheader->flag |= 0x1;
...
#if _PCM_SPEED_CHANGE||_PCM_PITCH_CHANGE
        if((AudioBufCtl.fifo_count+(PCM_AUDIO_BUF_SIZE-PCM_AUDIO_OUT_SIZE)-AudioBufCtl.process_count)>=0){
...
#endif    
    if(     AudioBufCtl.shadow_state!=STATUS_PLAYING
        &&  DacAssigner_codecUseDacCh(kPCM_CH1)>=0
    ){
		GIE_DISABLE();
		DacAssigner_unRegist(kPCM_CH1);
		GIE_ENABLE();
